(function () {
  ...
  function workspaceToolbar() {
    var a = state.actor;
    return '<div class="demo-toolbar saved-toolbar"><span class="demo-label">SAVED WORKSPACE</span><span class="workspace-account-label">' + e(roleLabel(a.role)) + (a.team ? ' · ' + e(a.team) : '') + '</span><div class="account-toolbar-actions"><button class="link-button" data-refresh>' + icon('refresh') + ' Refresh</button><button class="link-button" data-nav="profile">My profile</button><button class="link-button" data-logout>Sign out</button></div></div>';
  }
  ...
  function userForm(id, trigger) {
    if (state.actor.role !== 'official') return;
  ...
  function openCase(id, trigger) {
    ...
    api().then(function (data) { state = data; render(); show(); }).catch(error);
  }
  ...
  document.addEventListener('change', function (event) {
    ...
    if (['category-filter', 'priority-filter', 'status-filter'].indexOf(target.id) >= 0) {
