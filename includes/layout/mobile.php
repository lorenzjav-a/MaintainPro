...
<nav class="mobile-dock" aria-label="Quick navigation">
  ...
  <a href="complaints.php"<?= $activePage === 'complaints' ? ' class="active" aria-current="page"' : '' ?>><?= br_icon('inbox') ?><span><?= $resident ? 'My reports' : ($official ? 'Complaints' : 'Assignments') ?></span></a>
  <a class="mobile-primary" href="<?= $resident ? 'new-complaint.php' : ($official ? 'complaints.php?tab=assessment' : 'complaints.php?tab=work') ?>"><?= br_icon($resident ? 'plus' : ($official ? 'clipboard' : 'tool')) ?><span><?= $resident ? 'Report' : ($official ? 'Review' : 'My work') ?></span></a>
