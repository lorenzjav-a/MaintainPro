...
<div class="overview-grid">
  <div>
    ...
    <div class="bottom-panels">
      ...
      <section class="panel"><div class="panel-header"><h2 class="panel-title">Closing the loop</h2><?= br_icon('checkCircle') ?></div><div class="resolution-summary"><div class="completion-ring" style="--pct:<?= $ratio ?>%" role="img" aria-label="<?= $ratio ?> percent of complaints verified"><span><?= $ratio ?>%</span></div><div class="resolution-copy"><strong><?= $metrics['verified'] ?> of <?= $metrics['total'] ?> reports resident-verified</strong><p><?= $metrics['resolved'] ?> resolved and awaiting verification.<br>A completed action still needs the resident's confirmation.</p><a class="link-button" href="history.php">View resolution history <?= br_icon('arrow') ?></a></div></div></section>
    </div>
    <?php endif ?>
  </div>
  <aside class="side-stack">
    <?php if ($actor['role'] === 'official'): ?><section class="panel"><div class="panel-header"><h2 class="panel-title">Complaints by category</h2></div><?php br_chart($cases); ?><div class="chart-note">Top categories · <?= $metrics['total'] ?> total reports</div></section><?php endif ?>
