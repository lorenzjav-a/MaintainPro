...
<article class="panel detail-page" data-case-id="<?= h($c['id']) ?>" data-version="<?= (int)$c['version'] ?>">
  ...
  <div class="case-layout"><div class="case-main">
    ...
    <section class="case-section suggestion-box"><div class="eyebrow">SUGGESTED ACTIONS — FOR ASSESSMENT</div><?php if (!empty($c['suggestions'])): ?><ol><?php foreach ($c['suggestions'] as $i => $suggestion): ?><li><?= h($suggestion) ?><?= ($c['selectedSuggestion'] ?? null) === $i ? ' (reporter preference)' : '' ?></li><?php endforeach ?></ol><?php else: ?><p><?= h($c['suggestion'] ?: 'No earlier suggestion recorded.') ?></p><?php endif ?><p class="form-text">The official decides the recommendation; personnel document the actual work.</p></section>
    ...
    <?php foreach (array_reverse($c['timeline']) as $event): ?><div class="timeline-entry"><strong><?= h($event['title']) ?></strong><span class="timeline-date"><?= h(br_date($event['date'], true)) ?><br><?= h($event['actor']) ?></span><p><?= h($event['note']) ?></p>
